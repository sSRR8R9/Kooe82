...
async def inline_handler(event):  
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**rickthonbot"):
            buttons = [
                (
            ...
            builder = event.builder
            secret = os.path.join("./rickthon", "secrets.txt")
            try:
        ...
        result = types.InputBotInlineResult(
            ...
            url="https://dashboard.heroku.com/new?template=https://github.com/rick1128/rickthon",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
        ...
        await event.answer([result] if result else None)
@rickthon.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@rickthon.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
