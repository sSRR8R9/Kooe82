...
async def get_adzan(adzan):
    ...
    rickthonresult = f"<b>اوقـات صـلاه المـسلمين 👳‍♂️ </b>\
    ...
    "
    await edit_or_reply(adzan, rickthonresult, "html")
...
async def video_catfile(event):  # sourcery no-metrics
    ...
    await catevent.delete()
@rickthon.on(admin_cmd(pattern="تحويل ملف ([\s\S]*)"))
async def get(event):
    name = event.text[5:]
    if name is None:
    ...
    if m.text:
        ...
        await edit_or_reply(event, "**♛ ⦙ قم بالـرد على الرسالـة لتحويلها الى ملف**")
@rickthon.on(admin_cmd(pattern="بورن(?:\s|$)([\s\S]*)"))
...
@rickthon.on(admin_cmd(pattern="(طقس|الطقس)(?:\s|$)([\s\S]*)"))    
...
@rickthon.on(admin_cmd(pattern="تحويل رساله(?: |$)(.*)"))
async def get(event):
    reply = await event.get_reply_message()
    mediatype = media_type(reply)
    ...
    if os.path.exists(file_loc):
        os.remove(file_loc)
@rickthon.on(admin_cmd(pattern="تحويل ملف صوره(?: |$)(.*)"))
async def on_file_to_photo(event):
    ...
    try:
        ...
        return
    await catt.delete()
@rickthon.on(admin_cmd(pattern="تحويل ملصق متحرك(?:\s|$)([\s\S]*)"))
async def _(event):  # sourcery no-metrics
    ...
    for files in (catgif, catfile):
        if files and os.path.exists(files):
            os.remove(files)
@rickthon.on(admin_cmd(pattern="تحويل متحركه(?: |$)((-)?(r|l|u|d|s|i)?)$"))
async def pic_gifcmd(event):  # sourcery no-metrics
    reply = await event.get_reply_message()
    mediatype = media_type(reply)
    ...
    for i in [final, "Output.gif", imag[1]]:
        if os.path.exists(i):
            os.remove(i)
@rickthon.on(admin_cmd(pattern="مدينه الطقس(?:\s|$)([\s\S]*)"))    
...
@rickthon.on(admin_cmd(pattern=r"(ت(لي)?ج(راف)?) ?(م|ك|ميديا|كتابه)(?:\s|$)([\s\S]*)"))
async def _(event):
...
@rickthon.on(admin_cmd(pattern="تحويل فديو متحركه ?([0-9.]+)?$"))
async def _(event):
    reply = await event.get_reply_message()
    mediatype = media_type(event)
    if mediatype and mediatype != "video":
    ...
    for i in [inputfile, outputfile]:
        if os.path.exists(i):
            os.remove(i)
@rickthon.on(admin_cmd(pattern="طقوس(?:\s|$)([\s\S]*)"))    
async def _(event):
    reply_to_id = await reply_id(event)
    input_str = event.pattern_match.group(1)
    if not input_str:
