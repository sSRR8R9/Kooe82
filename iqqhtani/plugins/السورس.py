...
async def iq(rickthonevent):
    ...
    start = datetime.now()
    iqevent = await edit_or_reply(rickthonevent, "**♛︙ جاري فحص السورس **")
    end = datetime.now()
...
async def _(event):
    ...
    OUTPUT = (f"♛︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس تليثون @rickthon :**\n\n{o}")
            ...
            while flag:
                try:
                    ...
                    if check > 5:
                        break
            if BOTLOG:
                await rickthon.send_message(                    BOTLOG_CHATID,                    f"**♛︙   تحـميل المـلف 🗂️  : `{os.path.basename(downloaded_file_name)}`  تـم بنجـاح ✔️**",                )
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**♛︙  📑 يرجى متابع قناه الفارات تجدها هنا : @rickthon")
