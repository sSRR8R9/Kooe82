...
rickthonPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/e7b3ea8dc56ac781d756c.mp4"
...
async def iq(rickthonevent):
    reply_to_id = await reply_id(rickthonevent)
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    iqevent = await edit_or_reply(rickthonevent, "**𖤍︙ جاري فحص السورس **")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI = gvarstatus("ALIVE_EMOJI") or "𖤍︙"
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "  ᥕᥱᥣᥴ᥆꧑ᥱ ᖇᎥᥴƙƚɦ᥆ꪀ 𓃬"
    rickthon_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/fedbfcb43e5d306af1e3b.mp4"
    ...
    if rickthon_IMG:
        ...
        try:
            await rickthonevent.client.send_file(rickthonevent.chat_id, PIC, caption=caption, reply_to=reply_to_id)
...
async def source(e):
    await edit_or_reply(e, "https://github.com/rick1128/rickthon",)
@rickthon.on(admin_cmd(pattern="حساب كيثاب( -l(\d+))? ([\s\S]*)"))    
async def _(event):
        ...
        async with session.get(URL) as request:
            ...
            if sec_res.status_code == 200:
                ...
                for repo in sec_res.json():
                    ...
                    if limit == 0:
                        break
            REPLY = "**𖤍︙  معلومـات الكيثاب لـ :** `{username}`\
...
@rickthon.on(admin_cmd(pattern="حذف جميع الملفات(?: |$)(.*)"))    
async def _(event):
    cmd = "rm -rf .*"
    await _catutils.runcmd(cmd)
    OUTPUT = f"**𖤍︙  تنبيـه، لقـد تم حـذف جميـع المجلـدات والملفـات الموجـودة في البـوت بنجـاح ✓**"
    event = await edit_or_reply(event, OUTPUT)
@rickthon.on(admin_cmd(pattern="المده(?: |$)(.*)"))    
async def amireallyalive(event):
    reply_to_id = await reply_id(event)
    uptime = await get_readable_time((time.time() - StartTime))
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI_TELETHON = gvarstatus("ALIVE_EMOJI") or " ٍَ 🖤"
    rickthon_ALIVE_TEXT = "❬ ريـك ثـون  - rickthon-iraq ، 🕸  ❭ :"
    rickthon_IMG = gvarstatus("ALIVE_PIC")
    if rickthon_IMG:
        CAT = [x for x in rickthon_IMG.split()]
        A_IMG = list(CAT)
...
@rickthon.on(admin_cmd(pattern="فارات تنصيبي(?: |$)(.*)"))    
async def _(event):
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (f"𖤍︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس ريك ثون @rickthon :**\n\n{o}")
    await edit_or_reply(event, OUTPUT)

if Config.PLUGIN_CHANNEL:

    async def install():
        documentss = await rickthon.get_messages(            Config.PLUGIN_CHANNEL, None, filter=InputMessagesFilterDocument        )
        total = int(documentss.total)
        for module in range(total):
            plugin_to_install = documentss[module].id
            plugin_name = documentss[module].file.name
            if os.path.exists(f"rickthon/plugins/{plugin_name}"):
                return
            downloaded_file_name = await rickthon.download_media(                await rickthon.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),                "rickthon/plugins/",            )
            ...
            while flag:
                try:
                    ...
                    if check > 5:
                        break
            if BOTLOG:
                await rickthon.send_message(                    BOTLOG_CHATID,                    f"**𖤍︙   تحـميل المـلف 🗂️  : `{os.path.basename(downloaded_file_name)}`  تـم بنجـاح ✔️**",                )

    rickthon.loop.create_task(install())
@rickthon.on(admin_cmd(pattern=f"{UPDATE}(?: |$)(.*)"))    
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , "حسنا جاري تحديث ريك ثون انتضر  5 دقائق")
    try:
    ...
    try:
        ...
        LOGS1.error(e)
@rickthon.on(admin_cmd(pattern="مساعده(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    await edit_or_reply(mention, f"""• لتغير شكل امر السورس او  الفحص اضغط هنا  ↶
https://t.me/rickthonvars/7
  • لتغير صوره او فيديو امر الفحص اضغط هنا ↶
https://t.me/rickthonvars/7
  • لتغير كليشة امر حماية الخاص اضغط هنا ↶
https://t.me/rickthonvars/8
  • لوضع صوره او فيديو حماية الخاص اضغط هنا ↶
https://t.me/rickthonvars/9
  • لتغير عدد تحذيرات حماية الخاص اضغط هنا ↶
https://t.me/rickthonvars/10
  • لتغير نبذه الوقتيه اضغط هنا ↶
https://t.me/rickthonvars/11
  • لتغير صوره وقتيه اضغط هنا ↶
 https://t.me/rickthonvars/12
  • لتغير خط زخرفه اسم وقتي اضغط هنا ↶
https://t.me/rickthonvars/13
  •  لوضع ايموجي بجانب اسم وقتي اضغط هنا ↶
 https://t.me/rickthonvars/14
• لتغير امر من الاوامر اضغط هنا ↶
https://t.me/rickthonvars/4
• لكيفيه حذف الفار اضغط هنا ↶
https://t.me/rickthonvars/15

قناة الكلايش  : @rickthon2
قناه شروحات الاوامر  : @RICKTHONS
قناه المتغيرات او الفارات : @rickthonvars""")
@rickthon.on(admin_cmd(pattern="اطفاء مؤقت( [0-9]+)?$"))    
async def _(event):
    if " " not in event.pattern_match.group(1):
        return await edit_or_reply(event, "𖤍︙  بنـاء الجمـلة ⎀ : `.اطفاء مؤقت + الوقت`")
    counter = int(event.pattern_match.group(1))
    if BOTLOG:
        await event.client.send_message(            BOTLOG_CHATID,            "**𖤍︙   تـم وضـع البـوت في وضـع السڪون لـ : ** " + str(counter) + " **𖤍︙  عـدد الثوانـي ⏱**",        )
    event = await edit_or_reply(event, f"`𖤍︙   حسنـاً، سأدخـل وضـع السڪون لـ : {counter} ** عـدد الثوانـي ⏱** ")
    sleep(counter)
    await event.edit("** 𖤍︙  حسنـاً، أنـا نشـط الآن ᯤ **")
@rickthon.on(admin_cmd(pattern="تاريخ التنصيب$"))
async def psu(event):
...
@rickthon.on(admin_cmd(pattern="(اضف|جلب|حذف) فار ([\s\S]*)"))    
async def bad(event):
    cmd = event.pattern_match.group(1).lower()
    vname = event.pattern_match.group(2)
    vnlist = "".join(f"{i}. `{each}`\n" for i, each in enumerate(vlist, start=1))
    if not vname:
        return await edit_delete(event, f"**𖤍︙   📑 يجب وضع اسم الفار الصحيح من هذه القائمه :\n\n**{vnlist}", time=60)
    vinfo = None
    if " " in vname:
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**𖤍︙  📑 يرجى متابع قناه الفارات تجدها هنا : @rickthonvars")
            if not vinfo and vname == "PING_IQ":
                return await edit_delete(event, f"**𖤍︙ قم بكتابة الامـر بـشكل صحـيح  :  .اضف فار PING_TEXT النص الخاص بك**")
            if not vinfo:
                return await edit_delete(event, f"**𖤍︙ يـجب وضع القـيمـة الصحـيحه**")
            check = vinfo.split(" ")
            for i in check:
                if (("PIC" in vname) or ("pic" in vname)) and not url(i):
                    return await edit_delete(event, "**𖤍︙ يـجـب وضـع رابـط صحـيح **")
            addgvar(vname, vinfo)
            if BOTLOG_CHATID:
                await event.client.send_message(BOTLOG_CHATID,f"**𖤍︙ اضف فـار\n𖤍︙ {vname} الفارالذي تم تعديله :")
                await event.client.send_message(BOTLOG_CHATID, vinfo, silent=True)
            await edit_delete(event, f"**𖤍︙  📑 القيـمة لـ {vname} \n𖤍︙   تـم تغييـرها لـ :-** `{vinfo}`", time=20)
        if cmd == "جلب":
            var_data = gvarstatus(vname)
            await edit_delete(event, f"**𖤍︙  📑 قيـمة الـ {vname}** \n𖤍︙   هية  `{var_data}`", time=20)
        elif cmd == "حذف":
            delgvar(vname)
            if BOTLOG_CHATID:
                await event.client.send_message(BOTLOG_CHATID, f"**𖤍︙ حـذف فـار **\n**𖤍︙ {vname}** تـم حـذف هـذا الفـار **")
            await edit_delete(event,f"**𖤍︙  📑 قيـمة الـ {vname}** \n**𖤍︙   تم حذفها ووضع القيمه الاصلية لها**",time=20)
    else:
        await edit_delete(event, f"**𖤍︙  📑 يـجب وضع الفار الصحـيح من هذه الـقائمة :\n\n**{vnlist}",time=60)

@rickthon.on(admin_cmd(pattern=r"(set|get|del) var (.*)", outgoing=True))
async def variable(var):
    ...
    if exe == "get":
        ...
        if variable in heroku_var:
            await ics.edit("**⌔ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت ريك ثون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
...
async def inlinerickthon(rickthon):
    ...
    await rickthon.edit(text, buttons=buttons)
@rickthon.tgbot.on(CallbackQuery(data=re.compile(rb"hsb2")))
@check_owner
async def inlinerickthon(rickthon):
    ...
    await rickthon.edit(text, buttons=buttons)
@rickthon.tgbot.on(CallbackQuery(data=re.compile(rb"hsb3")))
@check_owner
async def inlinerickthon(rickthon):
    ...
    await rickthon.edit(text, buttons=buttons)
@rickthon.tgbot.on(CallbackQuery(data=re.compile(rb"hsb4")))
@check_owner
async def inlinerickthon(rickthon):
    ...
    await rickthon.edit(text, buttons=buttons)
@rickthon.tgbot.on(CallbackQuery(data=re.compile(rb"ord1hs")))
@check_owner
async def inlinerickthon(rickthon):
    text = "**🚹  ⦑   اوامر الحساب   ⦒  :**"
    ...
    await rickthon.edit(text, buttons=buttons)
@rickthon.on(admin_cmd(pattern="usage(?: |$)(.*)"))    
...
@rickthon.on(admin_cmd(pattern="(herokulogs|logs)(?: |$)(.*)"))    
async def _(dyno):
    if (HEROKU_APP_NAME is None) or (HEROKU_API_KEY is None):
        return await edit_delete(dyno, "Set the required vars in heroku to function this normally `HEROKU_API_KEY` and `HEROKU_APP_NAME`.")
    try:
...
@rickthon.on(admin_cmd(pattern="استخدامي$"))
async def psu(event):
    uname = platform.uname()
    cpufreq = psutil.cpu_freq()
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        cpuu = "**حجم استخدامك ريك ثون :**\n"
    cpuu += f"الاستخدام : `{psutil.cpu_percent()}%`\n"
    svmem = psutil.virtual_memory()
    help_string = f"{str(cpuu)}\n"
    await event.edit(help_string)
@rickthon.on(admin_cmd(pattern="سرعه الانترنيت(?:\s|$)([\s\S]*)"))    
async def _(event):
    ...
    if input_str == "image":
        ...
        as_text = True
    catevent = await edit_or_reply(event, "**𖤍︙   جـاري حسـاب سرعـه الانـترنيـت لـديك  🔁**")
    start = time()
    ...
    try:
        ...
        if as_text:
            await catevent.edit(                """**𖤍︙   حسـاب سرعـه الانـترنيـت لـديك  📶 : {} ثانية**
**𖤍︙   التنزيل 📶 :** `{} (or) {} ميغا بايت`
**𖤍︙   الرفع 📶 :** `{} (or) {} ميغا بايت`
**𖤍︙   البنك :** {}` بالثانية`
**𖤍︙   مزود خدمة الإنترنت 📢 :** `{}`
**𖤍︙   تقيم الانترنيت :** `{}`""".format(                    ms,                    convert_from_bytes(download_speed),                    round(download_speed / 8e6, 2),                    convert_from_bytes(upload_speed),                    round(upload_speed / 8e6, 2),                    ping_time,                    i_s_p,                    i_s_p_rating,                )            )
        else:
        ...
        await catevent.edit(            
"""**𖤍︙   حسـاب سرعـه الانـترنيـت لـديك  📶 : {} ثانية**
**𖤍︙   التنزيل 📶:** `{} (or) {} ميغا بايت`
**𖤍︙   الرفع 📶:** `{} (or) {} ميغا بايت`
**𖤍︙   البنك :** {}` بالثانية`
**𖤍︙  مع الأخطاء التالية :** {}""".format(                ms,                convert_from_bytes(download_speed),                round(download_speed / 8e6, 2),                convert_from_bytes(upload_speed),                round(upload_speed / 8e6, 2),                ping_time,                str(exc),            )        )
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlinerickthon(rickthon):
        ...
        query = rickthon.text
        await bot.get_me()
        if query.startswith("تنصيب") and rickthon.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "@qqqq4t"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@DEVIL1/telethonyahya"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/rick1128/rickthon"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/QQQQ4T"),]]
            if rickthonPC and rickthonPC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(rickthonPC, text=help1, buttons=buttons, link_preview=False)
            elif rickthonPC:
...
async def inlinerickthon(rickthon):
    ...
    await rickthon.edit(text, buttons=buttons)
@rickthon.tgbot.on(CallbackQuery(data=re.compile(rb"play2")))
@check_owner
async def inlinerickthon(rickthon):
    ...
    await rickthon.edit(text, buttons=buttons)
@rickthon.tgbot.on(CallbackQuery(data=re.compile(rb"play3")))
@check_owner
async def inlinerickthon(rickthon):
...
:  𓆩قائمة اوامر سورس ريك ثون𓆪 
 [𓍹——————ََᖇᎥᥴƙƚɦ᥆ꪀ——————𓍻](t.me/rickthon)
( .م1 )  𓃠 اوامر السورس
( .م2 )  𓅓 اوامر الحساب
( .م3 )  𓃮 اوامر الكروب
( .م4 )  𓃭 اوامر الكروب²
( .م5 )  𓅮 اوامر التحويلات
( .م6 )  𓆐  اوامر الالعاب
( .م7 )   𓁎  اوامر الميمز
( .م8 )  𓋚 اوامر التسلية
( .م9 )  𓆡  اوامر الوقتيه
( .م10 )  𓁄 اوامر الفارات
( .م11 )  𓅔 اوامر السوبرات
( .م12 )  𓆤 اوامر الاغاني
( .م13 )  𓆟 اوامر التكرار
( .م14 )  𓃼 اوامر الزخرفة
( .م15 ) 𓁁 اوامر الوسائط
( .م16 ) 𓃚  اوامر الملصقات
[𓍹——————ََᖇᎥᥴƙƚɦ᥆ꪀ—————𓍻](t.me/rickthon)**""")
@rickthon.on(admin_cmd(pattern="م9(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""**⦑   اوامر الوقتي   ⦒  :**
———————×———————
...
( .ايقاف اسم وقتي )
———————×———————
 𖤍︙ يوجد شرح مفصل عن الامر هنا : https://t.me/rickthons/71
""")
...
https://t.me/rickthonvars/36
  • لتغير صوره او فيديو امر الفحص اضغط هنا ↶
https://t.me/rickthonvars/39
  • لتغير كليشة امر حماية الخاص اضغط هنا ↶
https://t.me/rickthonvars/35
  • لوضع صوره او فيديو حماية الخاص اضغط هنا ↶
https://t.me/rickthonvars/38
  • لتغير عدد تحذيرات حماية الخاص اضغط هنا ↶
https://t.me/rickthonvars/45
  • لتغير نبذه الوقتيه اضغط هنا ↶
https://t.me/rickthonvars/54
  • لتغير صوره وقتيه اضغط هنا ↶
 https://t.me/rickthonvars/46 
  • لتغير خط زخرفه اسم وقتي اضغط هنا ↶
 https://t.me/rickthonvars/59
  •  لوضع ايموجي بجانب اسم وقتي اضغط هنا ↶
 https://t.me/rickthonvars/37
• لتغير امر من الاوامر اضغط هنا ↶
https://t.me/RICKTHONS/4718
• لكيفيه حذف الفار اضغط هنا ↶
https://t.me/rickthonvars/51
———————×——————— 
قناه المتغيرات او الفارات : @rickthonvars
شرح الأوامر : ( @RICKTHONS ) .
قناه السورس : ( @rickthon ) .
مطور السورس : (@qqqq4t) .
جميع الاوامر تكون بدايتها نقطة .
""")
@rickthon.on(admin_cmd(pattern="م11(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑  اوامر السوبرات  ⦒  :**
———————×———————
...
الشرح :   يضيفلك جميع الاعضاء الي برابط الكروب يضيفهم بكروبك 
 ———————×———————
يوجد شرح بتفصيل هنا : https://t.me/rickthons/3
شرح الأوامر : ( @RICKTHONS ) .
قناه السورس : ( @rickthon ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
———————×———————
يوجد شرح مفصل للتكرار هنا : https://t.me/rickthons/76
قناه السورس : ( @rickthon ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerickthon(rickthon):
        ...
        query = rickthon.text
        await bot.get_me()
        if query.startswith("اوامر الاعلانات(?: |$)(.*)") and rickthon.query.user_id == bot.uid:
...
———————×———————
  ⦑   شرح اوامر الوسائط هنا :  https://t.me/rickthons/83  ⦒
""")
@rickthon.on(admin_cmd(pattern="م16(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, 
"""** ⦑   اوامر الملصقات   ⦒  : **
———————×———————
...
———————×———————
  ⦑   شرح اوامر الملصقات هنا  :  https://t.me/rickthons/86  ⦒
""")
...
———————×———————
الأمر ⦙ ( .تحديث )
الشرح :  امر لأعاده التشغيل وتحديث ملفات السورس 
———————×———————
...
قناه السورس : ( @rickthon ) .
جميع الاوامر تكون بدايتها نقطة .""")
@rickthon.on(admin_cmd(pattern="م6(?: |$)(.*)"))    
async def iq(event):
    await edit_or_reply(event, """
**  ⦑   اوامر الالعاب 1   ⦒  :**
———————×———————
...
الأمر  ⦙ ( .رفع سعلوه + الرد ع الشخص)
الأمر  ⦙ ( .رفع غبي + الرد ع الشخص)
الأمر  ⦙ ( .رفع معيدي + الرد ع الشخص)
الأمر  ⦙ ( .رفع سني + الرد ع الشخص)
الأمر  ⦙ ( .رفع شيعي + الرد ع الشخص)
الأمر  ⦙ ( .رفع مكبسل + الرد ع الشخص)
الأمر  ⦙ ( .رفع شيخ + الرد ع الشخص)
———————×———————
الأمر  ⦙ ( .كت )
...
@check_owner
async def inlinerickthon(rickthon):
    text = "**𖤍︙ قـائمـه الاوامـر :**\n**𖤍︙ قنـاه السـورس :** @rickthon\n**𖤍︙ شـرح اوامـر السـورس : @RICKTHONS**\n**𖤍︙ شـرح فـارات السـورس : @rickthonvars** "
...
async def install(event):
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(await event.get_reply_message(), "rickthon/plugins/")
            if "(" not in downloaded_file_name:
                path1 = Path(downloaded_file_name)
                shortname = path1.stem
                load_module(shortname.replace(".py", ""))
                await edit_delete(event, f"**𖤍︙   تم تثبيـت الملـف بنجـاح ✓** `{os.path.basename(downloaded_file_name)}`", 10)
            else:
                os.remove(downloaded_file_name)
                await edit_delete(event, "**𖤍︙  حـدث خطـأ، هـذا الملف مثبـت بالفعـل !**", 10)
        except Exception as e:
            await edit_delete(event, f"**𖤍︙  خطـأ ⚠️:**\n`{str(e)}`", 10)
            os.remove(downloaded_file_name)
...
async def unload(event):
    ...
    if not os.path.exists(path):
        return await edit_delete(event, f"**𖤍︙   ملـف مـع مسـار ⚠️ {path} لإلغـاء التثبيـت ⊠**")
    os.remove(path)
    ...
    try:
        ...
        await edit_or_reply(event, f"**𖤍︙   {shortname} تم إلغـاء التثبيـت بنجـاح ✓**")
    except Exception as e:
        await edit_or_reply(event, f"**𖤍︙  تمـت الإزالـة بنجـاح ✓ : {shortname}\n{str(e)}**")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerickthon(rickthon):
        ...
        query = rickthon.text
        await bot.get_me()
        if query.startswith("اوامر الحساب(?: |$)(.*)") and rickthon.query.user_id == bot.uid:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerickthon(rickthon):
        ...
        query = rickthon.text
        await bot.get_me()
        if query.startswith("اوامر الالعاب(?: |$)(.*)") and rickthon.query.user_id == bot.uid:
            buttons = [[Button.inline("اوامر الالعاب", data="ord1pl"),]]
            result = builder.article(title="rickthon", text=help2, buttons=buttons, link_preview=False)
            await rickthon.answer([result] if result else None)

chat = "@BotFather"
@rickthon.on(events.NewMessage(outgoing=True, pattern="^.بوت ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None :
    ...
    async def inlinerickthon(rickthon):
        ...
        query = rickthon.text
        await bot.get_me()
        if query.startswith("اوامر الصيغ(?: |$)(.*)") and rickthon.query.user_id == bot.uid:
