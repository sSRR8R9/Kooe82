...
async def do_pm_permit_action(event, chat):  
    ...
    if PM_WARNS[str(chat.id)] >= MAX_FLOOD_IN_PMS:
        ...
        if custompmblock is not None:
            ...
            USER_BOT_WARN_ZERO = f"**𖤍 ⦙  تـم تـحـذيـرڪ مـسـبـقـاً مـن الـتـڪـرار . تـم حـظـرڪ ، لا يـمـڪـنـڪ ازعـاج الـمـالـڪ !**"
        msg = await event.reply(USER_BOT_WARN_ZERO)
        await event.client(functions.contacts.BlockRequest(chat.id))
        the_message = f"**𖤍 ⦙  الـمـحـضـوريـن مـن الـخـاص : 📩**\n♛ ⦙   [{get_display_name(chat)}](tg://user?id={chat.id}) **تـم حـظـر الـمـسـتـخـدم .🗣🚫** \n**♛ ⦙   عـدد الـرسـائـل :** {PM_WARNS[str(chat.id)]}"
...
async def do_pm_options_action(event, chat):
    ...
    if event.sender_id == 1226408155:
            return
    if str(chat.id) not in PM_WARNS:
        text = "**𖤍 ⦙  اخـتـࢪ احـد الخـيـاࢪات فـي الأعـلى بـلا تـڪـࢪاࢪ ، وهـذا تـحـذيـࢪڪ الاخـيـࢪ !❕🤍**"
    ...
    await event.client(functions.contacts.BlockRequest(chat.id))
    the_message = f"**𖤍 ⦙  حـمـايـة الـخـاص  (الـبـرايـفـت)  : 📩** \n[{get_display_name(chat)}](tg://user?id={chat.id}) **تـم حـظـر الـمـسـتـخـدم .🗣🚫**\n**♛ ⦙  الـسـبـب ~> اسـتـمـر بـالـتـكـرار .♻️**"
...
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "**𖤍 ⦙   عـذرا ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🧸♥️**"
        return await event.answer(text, cache_time=0, alert=True)
    text = "**𖤍 ⦙  حـسـنـاً ، تـم ارسـال طـلـبـڪ بـنـجـاح 💕 . لا تـقـم بـأخـتـيـار خـيـار ثـانـي .**\n**♛ ⦙  سَــ يـتـم الـرد عـلـيـڪ عـنـد تَـفَـرُغ الـمـالـڪ . ♥️🧸**"
