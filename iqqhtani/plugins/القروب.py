...
async def startmute(event):
    if event.is_private:
        ...
        if user.id == 2133239425:
            return await edit_or_reply(event, "**- دي لا يمڪنني كتـم صديق العزيز لمبرمج السـورس **")
        userid = user.id
...
async def tmuter(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit(" ◈ ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @x7_cm**")
