...
def pack_name(userid, pack, is_anim):
    if is_anim:
        return f"rickthonbot_{userid}_{pack}_anim"
    return f"rickthonbot_{userid}_{pack}"
...
async def pic2packcmd(event):
    ...
    try:
        ...
        emoji = "▫️"
    chat = "@Stickers"
    name = "rickthonbot_" + "".join(        random.choice(list(string.ascii_lowercase + string.ascii_uppercase))
        for _ in range(16)    )
