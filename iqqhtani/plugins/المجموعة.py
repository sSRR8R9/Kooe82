...
async def get_chatinfo(event):
    ...
    try:
        ...
        try:
            ...
            await event.reply("**𖤍 ⦙   لم يتم العثور على المجموعة او القناة**")
            return None
        except ChannelPrivateError:
            await event.reply(
                "**𖤍 ⦙   لا يمكنني استخدام الامر من الكروبات او القنوات الخاصة**"
            )
            return None
        except ChannelPublicGroupNaError:
            await event.reply("**𖤍 ⦙   لم يتم العثور على المجموعة او القناة**")
            ...
            await event.reply("**𖤍 ⦙   رابط الكروب غير صحيح**")
            return None
    return chat_info
...
async def promote(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"𖤍 ⦙  ترقية  🆙\
            \n𖤍 ⦙  المستخدم  🚹 : [{user.first_name}](tg://user?id={user.id})\
...
async def demote(event):
    ...
    try:
        ...
        return await catevent.edit(NO_PERM)
    await catevent.edit("**𖤍 ⦙  تـم تنزيله من قائمه الادمنيه بنجاح  ✅**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"𖤍 ⦙   تنزيل_مشرف\
...
async def pin(event):
    ...
    if BOTLOG and not event.is_private:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"𖤍 ⦙   تثبيت  📌\
                \n𖤍 ⦙   تم تثبيت الرسالة بنجاح في الدردشة  📌\
                \n𖤍 ⦙  المستخدم  🚹 : {event.chat.title}(`{event.chat_id}`)\
                \n𖤍 ⦙  المحادثة  📜 : {is_silent}",
        )
...
async def pin(event):
    ...
    if not to_unpin and options != "all":
        return await edit_delete(
            event,
            "𖤍 ⦙   يرجى الرد على الرسالة التي تريد تثبيتها استخدم `.الغاء التثبيت للكل`  لالغاء تثبيت جميع الرسائل  📍",
            5,
        )
    try:
        if to_unpin and not options:
            ...
            return await edit_delete(
                event, "𖤍 ⦙   يرجى الرد على الرسالة التي تريد تثبيتها استخدم `.الغاء التثبيت للكل`  لالغاء تثبيت جميع الرسائل  📍", 5
            )
    except BadRequestError:
        return await edit_delete(event, NO_PERM, 5)
    except Exception as e:
    ...
    if BOTLOG and not event.is_private:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"**𖤍 ⦙   الـغاء التثبيت  ❗️ \
                \n**𖤍  ⦙   تم بنجاح الغاء التثبيـت في الدردشة  ✅ \
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    deleted_msg = f"**𖤍 ⦙   اخر {lim} رسائل محذوفة في هذه المجموعة  🗑 :**"
    if not flag:
        for msg in adminlog:
            ruser = (
...
async def _ban_person(event):
    ...
    if not user:
        return
    if user.id == 5582470474:
        return await edit_delete(event, "**𖤍 ⦙   عـذرا أنـة مبـرمج السـورس  ⚜️**")
    if user.id == 818514313:
        return await edit_or_reply(event, "**- دي لا يمڪنني كتـم مبرمج السـورس **")
    if user.id == event.client.uid:
        return await edit_delete(event, "𖤍 ⦙   عـذرا لا تسـتطيع حـظر شـخص")
    catevent = await edit_or_reply(event, "𖤍 ⦙   تـم حـظره بـنجاح")
    try:
    ...
    try:
        ...
        return await catevent.edit(
            "𖤍 ⦙   ليـس لـدي جـميع الصـلاحيـات لكـن سيـبقى محـظور"
        )
    if reason:
        await catevent.edit(
            f"𖤍 ⦙   المسـتخدم {_format.mentionuser(user.first_name ,user.id)} \n ♛ ⦙   تـم حـظره بنـجاح !!\n**♛ ⦙  السبب : **`{reason}`"
        )
    else:
        await catevent.edit(
            f"𖤍 ⦙   المسـتخدم {_format.mentionuser(user.first_name ,user.id)} \n ♛ ⦙   تـم حـظره بنـجاح ✅"
...
async def iq(event):
    ...
    if type_of_group == "قناه":
        descript = "♛ ⦙   هذه قناة إختبار أُنشئت بإستعمال ريك ثون"
    else:
        descript = "♛ ⦙   هذه المجموعه إختبار أُنشئت بإستعمال ريك ثون"
