...
async def pyZip(e):
    ...
    zip_ = f"{nem_}.zip" if nem_ else "rickthon_Zip.zip"
    password = pass_ if pass_ else "rickthon"
...
async def unmuterickthon(event):
    if event.is_private:
        replied_user = await event.client.get_entity(event.chat_id)
...
async def _(event):
    ...
    rickthonevent = await edit_or_reply(event, "**♛ ⦙  جاري تفليش مجموعتك أنتظر قليلآ 🚮**")
    ...
    async for user in event.client.iter_participants(event.chat_id):
        ...
        try:
            ...
            LOGS.info(str(e))
    await rickthonevent.edit(f"**♛ ⦙   تم بنجاح تفليش مجموعتك من {total} الاعضاء 🚮**")
    
async def ban_user(chat_id, i, rights):
    try:
        await rickthon(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in rickthon.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await rickthon.send_message(event.chat_id, f"{usrtxt}\n\n{msg}")
  ...
  if mode == "text_on_reply":
    ...
    async for usr in rickthon.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await rickthon.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""
